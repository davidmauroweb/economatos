            ...
            <div class="card mt-3">
                ...
                <table class="table table-sn table-hover">
                    ...
                    <tbody>
                        ...
                        <td>{{$a->name}}</td>
                        <td>{{$a->email}}</td>
                        <td><button type="button" class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#m{{$a->id}}"><i class="bi bi-pencil-square"></i></button>
                    
                        <div class="modal fade" id="m{{$a->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                            <h6 class="modal-title" id="exampleModalLabel">Modificar {{$a->name}}</h6>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <form action="{{route('usuarios.update', $a->id)}}" method="post">
                            @csrf
                            @method('put')
                            <div class="row mb-3">
                            <label for="name" class="col-md-4 col-form-label text-md-end">{{ __('Nombre') }}</label>
                            <div class="col-md-6">
                                <input id="name" type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{$a->name}}" required autocomplete="name" autofocus>
                                @error('name')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                        ...
                        <div class="row mb-3">
                            <label for="capita" class="col-md-4 col-form-label text-md-end">{{ __('Cápita') }}</label>
                            <div class="col-md-6">
                                <input id="capita" type="number" class="form-control @error('capita') is-invalid @enderror" name="capita" autocomplete="capita" value="{{$a->capita}}" autofocus>
                            </div>
                        </div>
                        <div class="row mb-3">
                            <label for="adm" class="col-md-4 col-form-label text-md-end">{{ __('Rol') }}</label>
                            <div class="col-md-6">                            
                            <div class="form-check">
                            <input class="form-check-input" type="radio" name="adm" id="flexRadioDefault1" value="1" checked>
                            <label class="form-check-label" for="flexRadioDefault1">
                                Municipio
                            </label>
                            </div>
                            <div class="form-check">
                            <input class="form-check-input" type="radio" name="adm" id="flexRadioDefault2" value="0">
                            <label class="form-check-label" for="flexRadioDefault2">
                                Economato
                            </label>
                            </div>
